...
| **태그** | v1-a (`tags`, `link_tags`) | **구현 완료** (수동 부착, §2.9) | `/tags*` 호출로 UI 구성 |
| **웹 푸시** | REMIND v2 | **구현 완료** (VAPID, §2.10) | SW 등록 + `/push/subscribe` |
...
| DELETE | `/folder/{id}` | 폴더 삭제 — **하위 링크도 함께 삭제(cascade)**. 최상위 PARA 폴더는 거부(400) |
...
| POST | `/link/bulk` | 일괄 삭제/이동 → `{requested, affected}` |

**일괄 작업 요청** (`POST /link/bulk`):
```ts
{ ids: [1,2,3], action: "DELETE" | "MOVE", folderId?: 5 }
```
- `DELETE` — 여러 링크 한번에 삭제 (가중치·태그 정션도 정리)
- `MOVE` — `folderId` 로 이동. **"모두 Archive" = ARCHIVE 폴더로 MOVE** (PARA 자동 적용)
- **본인 소유 + 존재하는 ID 만 처리**, 나머지는 조용히 건너뜀 → `{requested, affected}` 로 처리 개수 반환
...
| GET | `/recommendation-weights/users/{userName}/today?mode=youtube_ctx` | 유튜브 컨텍스트 (content_type 기준) |
| GET | `/recommendation-weights/users/{userName}/today?mode=domain_ctx&host=blog.naver.com` | 도메인 컨텍스트 (host 기준) |

**컨텍스트 모드 매칭 규칙** (REMIND_STRATEGY §3.3):
- `youtube_ctx` — `content_type == YOUTUBE` 인 저장 링크. `youtube.com`/`youtu.be`/`m.youtube.com` 모두 같은 유튜브로 취급. **`host` 파라미터 선택**(보내도 무시).
- `domain_ctx` — 같은 도메인(host 일치, `www.` 자동 정규화). **`host` 파라미터 필수**.
- 호스트는 저장·비교 양쪽에서 선행 `www.` 가 제거됨 → `youtube.com` ↔ `www.youtube.com` 동일.
- ⚠️ "지금 보는 영상과 유사" 가 아니라 "내가 저장한 유튜브 영상 전체(미열람·점수순)" 임. 같은 채널 매칭은 메타데이터(author) 필요 — v2 미구현.
...
> 디지스트는 백엔드 `@Scheduled` **매시 정각 틱**(`hourlyTick`)이 사용자별 prefs(시각/요일/타임존)에 맞춰 자동 INSERT + 웹푸시 발송한다. 일일은 `daily_time` 시(時)가 사용자 타임존 현재 시와 일치할 때, 주간은 `weekly_dow`+`weekly_time` 일치할 때 발사. 프론트는 GET만 하면 됨.
...
| Method | Path | 요약 |
|---|---|---|
| GET | `/me/reminder-prefs` | 리마인드 채널/시간/요일/타임존 |
| PUT | `/me/reminder-prefs` | 환경설정 변경 (보낸 필드만 갱신) |

**PUT 요청 (모든 필드 선택 — nullable, 보낸 것만 반영)**:
```ts
{
  dailyEnabled?: boolean,        // 일일 알림 on/off (기본 true)
  dailyTime?: "09:00",           // 일일 발송 시각 (LocalTime)
  timezone?: "Asia/Seoul",       // IANA tz
  weeklyEnabled?: boolean,       // 주간 요약 on/off (기본 true)
  weeklyDow?: 7,                 // 주간 요일 1=월 … 7=일 (기본 7)
  weeklyTime?: "21:00",          // 주간 발송 시각 (기본 21:00)
  emailEnabled?: boolean,        // 이메일 채널 (플래그만 — 발송 미구현)
  maxItemsPerReminder?: 5        // 1~20
}
```

### 2.9 태그 (사용자 자유 태그 — ERD §4.3)

| Method | Path | 요약 |
|---|---|---|
| POST | `/tags` | 태그 생성 `{name}` (사용자별 중복·대소문자 무시, 50자 제한) |
| GET | `/tags` | 내 태그 목록 (이름 오름차순) |
| DELETE | `/tags/{tagId}` | 태그 삭제 (붙은 연결도 정리) |
| POST | `/tags/{tagId}/links/{linkId}` | 링크에 태그 부착 (멱등) |
| DELETE | `/tags/{tagId}/links/{linkId}` | 태그 해제 |
| GET | `/tags/{tagId}/links` | 태그로 링크 필터 → `LinkResponse[]` |
| GET | `/tags/by-link/{linkId}` | 특정 링크에 달린 태그 → `TagResponse[]` |

- **수동 방식**: 사용자가 직접 만들고 붙인다. 자동 태깅/분류 추천은 미구현.
- **TagResponse**: `{ id, name, createdAt }`
- 폴더(PARA)와 독립된 자유 검색/필터 축 (ERD 설계 노트 #3).

### 2.10 웹 푸시 (OS 알림 — REMIND v2 앞당겨 구현)

| Method | Path | 요약 |
|---|---|---|
| GET | `/push/vapid-public-key` | VAPID 공개키 (인증 불필요) |
| POST | `/push/subscribe` | 구독 등록 `{endpoint, keys:{p256dh, auth}}` |
| DELETE | `/push/subscribe` | 구독 해제 `{endpoint}` |
| POST | `/push/test` | 일일 다이제스트 테스트 발송 |
| POST | `/push/test-weekly` | 주간 요약 테스트 발송 |

- 흐름: `getVapidPublicKey` → `serviceWorker.register` + `pushManager.subscribe` → `POST /push/subscribe`
- 410/404 응답 시 백엔드가 죽은 구독을 자동 삭제.
- ⚠️ FCM 차단 네트워크(일부 사내망)에선 전달 안 됨 — 코드 무관 환경 이슈.
...
|---|---|---|
| `folders` | `folder` | `para_category` ✓ + `parent_id` 자기참조(PARA 2단) |
| `links` | `link_data` | `host`(www. 정규화)/`content_type`/`thumbnail_url` ✓ |
| `tags` / `link_tags` | `tags` / `link_tags` ✓ | ERD 구조 정렬 (uuid→int, RLS→소유권 검증) |
| `link_reminders` | `link_reminders` | `channel`/`mode`/`opened_at`/`snoozed_until` ✓ |
| `user_reminder_prefs` | `user_reminder_prefs` | +`weekly_dow`/`weekly_time` (문서 초과) |
| (없음) | `recommendation_weight` | save-it ERD에 없음 — 가중치/스누즈/importance 통합 (문서 `links.priority` 대체) |
| (없음) | `push_subscriptions` | 웹 푸시 구독 (REMIND v2) |
| (없음) | `refresh_tokens` / `api_tokens` | JWT refresh + 익스텐션 토큰 (Supabase auth.users 대체) |

> ID 타입은 ERD의 `uuid` → 우리 `int`/`long`. 보안은 Supabase RLS → JWT 기반 `SecurityUtil.currentUserId()` 소유권 검증으로 대체. 구조·관계는 동일.
...
| 자동 테스트 | 수동 통합 검증 위주 | ScoringEngine JUnit 추가 |
...
| 항목 | 출처 | v1 처리 |
|---|---|---|
| 이메일 알림 | REMIND §4 (v1 채널) | `email_enabled` 플래그만 — 발송 로직 미구현 (Resend 등 연동 시 추가) |
| 같은 채널 유사 추천 | REMIND §3.3 (v2) | `author` 메타데이터 미수집 — 미구현 |
| Realtime 동기화 | 모바일 셸 specs "미포함 사항" | mutation 후 `router.refresh()` 또는 v2 푸시 |
| 다크 모드, 링크 편집 UI | 모바일 셸 specs | 별도 큐 |

> 태그 / link_tags(§2.9), 웹 푸시(§2.10), 일괄 작업(`POST /link/bulk`) 은 **구현 완료** — v1 제외 항목에서 빠짐.
...
# 4. 회귀 검증 (선택) — TEST_SCENARIOS.md 의 시나리오를 curl 로 확인
```

**디지스트 틱 확인** (개발용):
스케줄러는 **매시 정각**(`@Scheduled(cron="0 0 * * * *", zone="UTC")`) 돌며 사용자별
`reminder_prefs`(시각/요일/타임존)에 맞는 사용자에게만 발사한다. 즉시 테스트하려면
`POST /push/test`(일일) / `POST /push/test-weekly`(주간) 엔드포인트를 쓰는 게 빠르다.

> 일일 = `daily_time` 시(時)가 사용자 타임존 현재 시와 일치 / 주간 = `weekly_dow`+`weekly_time` 일치 시 발사.
